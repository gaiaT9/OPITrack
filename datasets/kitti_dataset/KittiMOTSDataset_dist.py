...
class MOTSTrackCarsTrain(Dataset):
    ...
    def get_data_from_mots(self, index):
        ...
        for pind, pis in enumerate(valid_samples):
            tids = []
            for pi in pis:
                # make time ID sub matrix (instance 2 instance)
                # record: (tid, num sample at tid)
                tids.append((pi[0]['tid'], len(pi)))
            ...
            # gen dist sub matrix
            seq_samples = sum([i[1] for i in tids])
            tid_sub_mat = np.zeros((seq_samples, seq_samples))
            accum_l_samples = 0
            for l_tid, l_num_samples in tids:
                accum_r_samples = 0
                for r_tid, r_num_samples in tids:
                    tid_sub_mat[accum_l_samples: accum_l_samples + l_num_samples, accum_r_samples: accum_r_samples + r_num_samples] = max(abs(r_tid - l_tid) - 1, 0)
                    accum_r_samples += r_num_samples
                    pass
                accum_l_samples += l_num_samples
            # normalize the matrix
            tid_sub_mat /= (tid_sub_mat.max() + 1)
            sample['tid_dist'].append(tid_sub_mat)

        # gen global dist sub matrix0
        total_shape = sum([i.shape[0] for i in sample['tid_dist']])
        # dist of instances between 2 seq is k
        k = 1
        global_tid_sub_map = np.ones((total_shape, total_shape)) * k
        accum_size = 0
        for m in sample['tid_dist']:
            size = m.shape[0]
            global_tid_sub_map[accum_size: accum_size + size, accum_size: accum_size + size] = m
            accum_size += size
        sample['tid_dist'] = global_tid_sub_map
