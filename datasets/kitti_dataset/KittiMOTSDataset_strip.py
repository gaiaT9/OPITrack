...
class MOTSTrackCarsValOffset(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, az=False,
                 border=False, env=False, gt=True, box=False, test=False, category=False, ex=0.2, hstrips=3, vstrips=3, 
                 hstrips_overlap=1/3, vstrips_overlap=1/3, fg_points=1000, bg_points=500):
        ...
        self.hstrips = hstrips
        self.vstrips = vstrips
        self.hstrips_overlap = hstrips_overlap
        self.vstrips_overlap = vstrips_overlap
    ...
    def get_data_from_mots(self, index):
        ...
        for inst_id in inds:
            ...
            # bg_num = int(self.num_points / (self.hstrips * self.vstrips + 1))
            # fg_num = int((self.num_points - bg_num) / (self.hstrips * self.vstrips))
            bg_num = self.bg_points
            fg_num = int(self.fg_points / (self.hstrips * self.vstrips))

            # filter noisy prediction use 3 delta (in Y direction)
            vs_, us_ = np.nonzero(mask)
            ymin = max(vs_.min(), vs_.mean() - vs_.std() * 3)
            ymax = min(vs_.max(), vs_.mean() + vs_.std() * 3)
            ymin = int(np.round(ymin))
            ymax = int(np.round(ymax))
            pmask = np.zeros_like(mask)
            pmask[ymin: ymax+1, :] = mask[ymin: ymax+1, :]
            mask = pmask
            ...
            # y_min = max(pointUVs[:, 3].min(), pointUVs[:, 3].mean() - pointUVs[:, 3].std() * 3)
            # y_max = min(pointUVs[:, 3].max(), pointUVs[:, 3].mean() + pointUVs[:, 3].std() * 3)
            # vertical
            y_min = pointUVs[:, 3].min()
            y_max = pointUVs[:, 3].max()
            vstep = (y_max - y_min) / self.vstrips
            y_range = vstep / (1 - self.vstrips_overlap)
            # horizenal
            x_min = pointUVs[:, 4].min()
            x_max = pointUVs[:, 4].max()
            hstep = (x_max - x_min) / self.hstrips
            x_range = hstep / (1 - self.hstrips_overlap)
            point_strips = []
            for j in range(self.hstrips):
                hrange_min = x_min + j * hstep
                hrange_max = hrange_min + x_range
                hvalid_idx = np.logical_and(pointUVs[:, 4] >= hrange_min, pointUVs[:, 4] <= hrange_max)
                for i in range(self.vstrips):
                    vrange_min = y_min + i * vstep
                    vrange_max = vrange_min + y_range
                    vvalid_idx = np.logical_and(pointUVs[:, 3] >= vrange_min, pointUVs[:, 3] <= vrange_max)
                    valid_idx = np.logical_and(hvalid_idx, vvalid_idx)
                    strip_points = pointUVs[valid_idx].astype(np.float32)
                    choice = np.random.choice(strip_points.shape[0], fg_num)
                    strip_fg = strip_points[choice][np.newaxis, :, :].astype(np.float32)
                    point_strips.append(strip_fg)

            point_strips = np.concatenate(point_strips, axis=1)
            point_strips = np.concatenate(
                [point_strips, np.zeros((point_strips.shape[0], point_strips.shape[1], 3), dtype=np.float32)], axis=-1)
            ...
            sample['points'].append(np.concatenate([point_strips, points_bg], axis=1))
...
class MOTSTrackPersonValOffset(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, az=False,
                 border=False, env=False, gt=True, box=False, test=False, category=False, ex=0.2, strips=6):
...
class MOTSTrackCarsTrain(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, batch_num=8,
                 shift=False, size=3000, sample_num=30, nearby=1, trainval=False, category=False, hstrips=3, vstrips=3, 
                 hstrips_overlap=1/3, vstrips_overlap=1/3, fg_points=1000, bg_points=500,
                 seq_sample_num=-1, per_seq_sample_num=2):
        print('MOTS Dataset created')
        type = 'training' if type in 'training' else 'testing'
        if trainval:
...
class MOTSTrackPersonTrain(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, batch_num=8,
                 shift=False, size=3000, sample_num=30, nearby=1, trainval=False, category=False, strips=6):
        print('MOTS Dataset created')
        type = 'training' if type in 'training' else 'testing'
        if trainval:
            self.squence = self.SEQ_IDS_TRAIN + self.SEQ_IDS_VAL
