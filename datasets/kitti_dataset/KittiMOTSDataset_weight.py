...
class MOTSTrackCarsTrain(Dataset):
            ...
            for fi, f in fs.items():
                ...
                for inst in f:
                    ...
                    newDict = {'frame': frameCount, 'sp': inst['sp'], 'img': inst['img'], 'mask': inst['mask'], 'maskX': inst['maskX'],
                               'iou': inst['iou'], 'area_1order': inst['area_1order'], 'largest_area': inst['largest_area'], 
                               'breakpoint_dist': inst['breakpoint_dist'],}
    ...
    def get_data_from_mots(self, index):
        ...
        sample['area_weights'] = []
        sample['ious'] = []
        sample['breakpoint_dist'] = []
        # label of negative sample: -1
        inst_id_mapping = {'negative': -1}
        current_id = 1
        # for every seq
        for pind, pis in enumerate(valid_samples):
            total_samples = sum(len(i) for i in pis)
            seq_iou_matrix = np.zeros((total_samples, total_samples))
            seq_sample_info = []
            # for every frame in seq
            for pi in pis:
                ...
                for ii, inst in enumerate(pi):
                    ...
                    sample['envs'].append(fg_num)
                    # get normalized area 1-order diff
                    normalized_1order = (inst['area_1order'] / (inst['largest_area'] + np.spacing(1)))
                    sample['area_weights'].append(normalized_1order)
                    # gen local iou matrix
                    current_info = (inst['frame'], inst['inst_id'].split('_')[-1])
                    for iou_info in inst['iou']:
                        iou_value, o_frame, o_id = iou_info
                        o_info = (o_frame, str(o_id))
                        if o_info in seq_sample_info:
                            seq_iou_matrix[len(seq_sample_info) - 1, seq_sample_info.index(o_info)] = iou_value
                    seq_sample_info.append(current_info)
                    # get breakpoint dist
                    sample['breakpoint_dist'].append(inst['breakpoint_dist'])
            sample['ious'].append(seq_iou_matrix)
        ...
        sample['xyxys'] = np.array(sample["xyxys"], dtype=np.float32)
        # gen weight matrix
        # area weights with shape (n_sample, )
        sample['area_weights'] = np.array(sample['area_weights'], dtype=np.float32)
        # full iou matrix with shape (n_sample, n_sample)
        full_iou_matrix = np.zeros((len(sample['points']), len(sample['points'])))
        current_idx = 0
        for m in sample['ious']:
            # paste chunk to main matrix, because m is a lower triangular matrix, use m + m.T to make iou relevan matrix
            full_iou_matrix[current_idx: current_idx + m.shape[0], 
                            current_idx: current_idx + m.shape[1]] = m + m.T
        sample['ious'] = full_iou_matrix
        # breakpoint dist
        sample['breakpoint_dist'] = np.array(sample['breakpoint_dist'], dtype=np.float32)
        return sample
...
class MOTSTrackPersonTrain(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", num_points=250, transform=None, random_select=False, batch_num=8,
        ...
        self.transform = transform
        db_dir = kittiRoot + 'PersonsPredTrackEnvDB/'
