...
def compute_MOTS_metrics(gt, results, max_frames, class_id, ignore_class, overlap_function, logger=print):
    """
    ...
    print_summary(list(gt.keys()), results_per_seq, results_for_all_seqs, logger=logger)
...
def print_summary(seq_names, results_per_seq, results_for_all_seqs, column_width=14, logger=print):
    ...
    logger(row_format.format("", *metrics_names))
    ...
    logger(row_format.format("all", *all_results))
    for seq in seq_names:
        all_results = format_results_entries(results_per_seq[seq])
        logger(row_format.format(seq, *all_results))
...
def load_sequences(path, seqmap, logger=print):
    objects_per_frame_per_sequence = {}
    for seq in seqmap:
        logger("Loading sequence " + seq)
...
def evaluate_class(gt, results, max_frames, class_id, logger=print):
  _, results_obj = compute_MOTS_metrics(gt, results, max_frames, class_id, IGNORE_CLASS, mask_iou, logger=logger)
  return results_obj
