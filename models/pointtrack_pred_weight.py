...
class PoseNetFeatOffsetEmb(nn.Module):
    ...
    def forward(self, inp, borders, spatialEmbs, with_weight=False):
        ...
        # if self.training:
        #     zero_count = (torch.abs(weightFeat.squeeze(1)) < 1e-4).sum(-1)
        #     nonzero_count = weightFeat.shape[-1] - zero_count
        #     avg_nonzero = (nonzero_count / weightFeat.shape[-1]).mean()
        #     open('weight.log', 'a').write('%.4f\n'%avg_nonzero.item())
        ...
        # add magnitude normalize could be better?
        x = torch.cat([ap_x1, mp_x2, weight_x3, border_feat, spatialEmbs], dim=1)
        outp = self.last_emb(x)
        return outp
