...
class PoseNetFeatOffsetEmb(nn.Module):
    ...
    def __init__(self, num_points=250, ic=7, border_points=200, border_ic=6, output_dim=64, category=False):
        ...
        self.ap_emb = nn.Sequential(
            nn.Linear(64, 64),
            nn.LeakyReLU(),
            nn.Linear(64, output_dim)
        )

        self.mp_emb = nn.Sequential(
            # nn.Linear(64, 64),
            # nn.LeakyReLU(),
            nn.Linear(64, output_dim)
        )
    ...
    def forward(self, inp, borders, spatialEmbs, with_weight=False):
        ...
        component_out = [
            self.ap_emb(ap_x1),
            # self.mp_emb(mp_x2),
            self.weight_emb(weight_x3),
            self.bg_emb(border_feat),
            self.sp_emb(spatialEmbs)
        ]

        return outp, component_out
...
class TrackerOffsetEmb(nn.Module):
    ...
    def forward(self, points, labels, xyxys, infer=False, visualize=False):
        ...
        if infer:
            ...
            embeds, component_out = self.point_feat(points.transpose(2, 1).contiguous(), envs.transpose(2, 1).contiguous(), embeds)
            labels = labels[0]
            # l_trip = self.compute_triplet_loss(embeds, labels)
            l_emb = self.compute_triplet_loss(embeds, labels)
            l_comp = sum([self.compute_triplet_loss(e, labels) for e in component_out])
            # l_reg = self.calc_mp_regularzation()
            # fp = open('test.log', 'a')
            # fp.write(str(l_reg.item()))
            # fp.write('\n')
            # return l_trip + l_reg
            return l_emb# + 0.5*l_comp

    def inference(self, points, envs, embeds):
        # assert points.shape[0] == 1
        embeds, _ = self.point_feat(points.transpose(2, 1).contiguous(), envs.transpose(2, 1).contiguous(), embeds)
        return embeds
