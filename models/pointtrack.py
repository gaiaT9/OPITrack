...
class PoseNetFeatOffsetEmb(nn.Module):
    ...
    def forward(self, inp, borders, spatialEmbs, with_weight=False):
        ...
        mp_x2, mp_idx = x2.max(-1, keepdim=True)
        if self.training:
            mean_idx = 0
            for i in range(mp_idx.shape[0]):
                mean_idx += torch.unique(mp_idx[i]).shape[0]
            mean_idx /= mp_idx.shape[0]
            open('mp_idx.log', 'a').write("%.4f\n" % mean_idx)
        else:
            mean_idx = 0
            for i in range(mp_idx.shape[0]):
                mean_idx += torch.unique(mp_idx[i]).shape[0]
            mean_idx /= mp_idx.shape[0]
            open('mp_idx_test.log', 'a').write("%.4f\n" % mean_idx)
        mp_x2 = mp_x2.squeeze(-1)
        ...
        if self.training:
            ...
            open('weight_test.log', 'a').write('%.4f\n'%weight.std().item())
