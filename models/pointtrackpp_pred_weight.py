...
from .pointtrack import PointFeatFuse3P


class PoseNetFeatOffsetEmbPP(nn.Module):
    """
    PointTrack++ network arch
    """
    # bn with border
    def __init__(self, num_points=250, ic=7, border_points=200, border_ic=6, output_dim=64, category=False):
        super(PoseNetFeatOffsetEmbPP, self).__init__()
        ...
        self.mf_mlp = nn.Sequential(
            nn.Linear(64*2 + 512, 256),
            nn.BatchNorm1d(256),
            nn.LeakyReLU(),
            nn.Linear(256, 64),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(),
            nn.Linear(64, output_dim // 4)
            # nn.Linear(64, 8 * 2)
        )

        self.me_mlp = nn.Sequential(
            nn.Linear(256, 128),
            nn.BatchNorm1d(128),
            nn.LeakyReLU(),
            nn.Linear(128, 64),
        ...
        self.ma_mlp = nn.Sequential(
            nn.Linear(output_dim // 4 * 3, output_dim // 4),
            # nn.Linear(22, 5 * 2)
        )
    ...
    def forward(self, inp, borders, spatialEmbs, with_weight=False):
        ...
        # x = torch.cat([ap_x1, mp_x2, weight_x3, border_feat, spatialEmbs], dim=1)
        # outp = self.last_emb(x)
        mf = torch.cat([ap_x1, mp_x2, weight_x3], dim=1)
        mf = self.mf_mlp(mf)
        me = self.me_mlp(border_feat)
        mp = self.mp_mlp(spatialEmbs)
        ma = self.ma_mlp(torch.cat([mf, me, mp], dim=1))

        outp = torch.cat([mf, ma, me, mp], dim=1)
        return outp



class TrackerOffsetEmbPP(nn.Module):
    ...
    def __init__(self, margin=0.3, num_points=250, border_ic=6, env_points=200, category=False, outputD=64, v23=False):
        super().__init__()
        self.point_feat = PoseNetFeatOffsetEmbPP(num_points=num_points, ic=3, border_points=env_points, border_ic=border_ic, output_dim=outputD, category=True)
        self.num_points = num_points
        self.margin = margin
    ...
    def compute_triplet_loss(self, inputs, targets, vec_weight=None, corr_weight=None, breakpoint_weight=None):
        """
        Args:
            inputs: feature matrix with shape (num_samples, feat_dim)
            targets: ground truth id labels with shape (num_samples)
        """
        n = inputs.size(0)
        ...
        # dist = dist * (1 + corr_weight[0] > 0.3)
        dist = dist * torch.exp(-corr_weight[0])
        # valid_weight = (breakpoint_weight > 0).float() * (breakpoint_weight < 3).float()
        # cross_weight = torch.matmul(valid_weight.T, valid_weight)
        # dist = dist * (torch.exp(-cross_weight))
        ...
        if mask.float().unique().shape[0] > 1:
            ...
            for i in range(n):
                ...
                # dist_ap.append(dist[i][mask[i]].max().unsqueeze(0) * torch.exp(vec_weight[0][i]))
                dist_an.append(dist[i][mask[i] == 0].min().unsqueeze(0))# * torch.exp(-corr_weight[0][i][mask[i] == 0][dist[i][mask[i] == 0].argmin()]))
    ...
    def forward(self, points, labels, xyxys, infer=False, visualize=False, vec_weight=None, corr_weight=None, breakpoint_weight=None):
        ...
        if infer:
            ...
            labels = labels[0]
            return self.compute_triplet_loss(embeds, labels, vec_weight=vec_weight, corr_weight=corr_weight, breakpoint_weight=breakpoint_weight)
