...
class PoseNetFeatOffsetEmb(nn.Module):
    # bn with border
    def __init__(self, num_points=250, ic=7, border_points=200, border_ic=6, output_dim=64, category=False, strips=6):
        ...
        self.last_share_head = nn.Sequential(
            nn.Linear(704+bc, 256),
            nn.LeakyReLU(),
            nn.Linear(256, output_dim)
            )

        # self.last_emb = nn.ModuleList()

        # for _ in range(self.strips):
        #     self.last_emb.append(
        #         nn.Sequential(
        #             # nn.Linear(704+bc, 256),
        #             # nn.LeakyReLU(),
        #             nn.Linear(256, output_dim)
    ...
    def forward(self, inp, borders, spatialEmbs, with_weight=False):
        ...
        point_strips = torch.chunk(inp, self.strips, dim=-1)

        for i in range(self.strips):
            # range_min = y_min + i * step
            # range_max = range_min + y_range

            # valid_idx = torch.logical_and(inp[:, -2] > range_min, inp[:, -2] < range_max)
            # valid_idx = valid_idx.unsqueeze(1).expand(inp.shape)

            # range_inp = inp[valid_idx]
            range_inp = point_strips[i]

            ap_x1, mp_x2, weight_x3 = self.share_forward(range_inp)

            ap_x1s.append(ap_x1)
            mp_x2s.append(mp_x2)
            weight_x3s.append(weight_x3)

            x = torch.cat([ap_x1, mp_x2, weight_x3, border_feat, spatialEmbs], dim=1)
            outp = self.last_share_head(x)
            # outp = self.last_emb[i](x)
            # outp = self.last_emb(x)
...
class TrackerOffsetEmb(nn.Module):
    ...
    def __init__(self, margin=0.3, num_points=250, border_ic=6, env_points=200, category=False, outputD=64, v23=False, strips=6):
        super().__init__()
        self.point_feat = PoseNetFeatOffsetEmb(num_points=num_points, ic=3, border_points=env_points, border_ic=border_ic, output_dim=outputD, 
                                               category=True, strips=strips)
        self.num_points = num_points
        self.margin = margin
    ...
    def forward(self, points, labels, xyxys, infer=False, visualize=False):
        ...
        envs = points[:,self.num_points * self.strips:]
        points = points[:,:self.num_points * self.strips, :5]
        if infer:
            ...
            labels = labels[0]
            losses = torch.zeros((1, ), device=points.device).type(points.dtype)
            for i in range(self.strips):
                losses += self.compute_triplet_loss(embeds[i], labels)
            # losses = self.compute_triplet_loss(embeds[0], labels)
            return losses
