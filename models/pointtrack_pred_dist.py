...
class TrackerOffsetEmb(nn.Module):
    ...
    def compute_triplet_loss(self, inputs, targets, dist_mat=None):
        """
        Args:
        ...
        # dist factor: current 1 / (d + 1)
        dist_mat = dist_mat[0] * mask
        factor = 1 / (dist_mat + 1) + 1
        factor[mask == 0] = 2
        # factor = 1 / dist
        # dist = factor * dist
    ...
    def forward(self, points, labels, xyxys, infer=False, visualize=False, dist_mat=None):
        ...
        if infer:
            ...
            labels = labels[0]
            return self.compute_triplet_loss(embeds, labels, dist_mat=dist_mat)
