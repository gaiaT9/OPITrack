...
class TrackHelper(object):
    ...
    def __init__(self, save_dir, margin, t_car=0.8165986526897969, t_person=0.47985540892434836, alive_car=5, alive_person=30, car=True,
        ...
        if car:
            ...
            self.reid_euclidean_scale = 1
            self.reid_euclidean_offset = 2
            ...
            # self.reid_euclidean_scale = 1
            # self.reid_euclidean_offset = 0.2
            self.association_threshold = t_person
            # self.association_threshold = 0.975
    ...
    def export_last_video(self, verbose=False, export_features=True):
        if verbose:
        ...
        if export_features:
            ...
            import pickle
            processed_elements = []
            for o in self.all_tracks:
                processed_elements.append({'t': o.t, 'class_id': o.class_id, 'track_id': o.track_id, 'mask': o.mask, 'embed': o.embed})
            pickle.dump(processed_elements, open(os.path.join('features', self.current_video + ".pkl"), 'wb'))
    ...
    def tracking(self, subfolder, frameCount, embeds, masks, verbose=False):
        ...
        if not subfolder == self.current_video:
            # a new video
            self.export_last_video(verbose=verbose)
